...
const L = {
  pt: {
    ...
    fSemStripeCorpo: 'Sem a chave da Stripe não é possível saber o que entrou. Os números de receita acima usam o valor CONTRATADO, não o recebido.',
  ...
  en: {
    ...
    existingNote: 'This email already has an account. The invite becomes a second membership: the current password stays valid and the person picks the company at login.',
    ...
    gateForbiddenTitle: 'This is not a platform account', gateForbiddenBody: 'You are on a company account. Sign out and use the Soluqtion account.',
    storageQuota: 'Evidence storage', storageWarn: 'Once the cap is reached, the whistleblowing and consumer-service channels stop accepting ATTACHMENTS — reports still come in, but with no photo, video or document.',
    ...
    fPorMes: 'Month', fPorMesLegenda: 'Income is gross; costs include AI, Stripe fees and fixed infrastructure. Storage counts only in the current month, because the database stores the size as of today, not the history.',
    ...
    fChurn: 'cancelling', fChurnSub: 'Already cancelled, with access until the cycle ends. This is the MRR that drops next month.',
    ...
    fRateio: 'Allocation of measured usage — no provider bills us per company.',
    ...
    aiTitle: 'AI tokens', aiSub: 'How much of its granted quota each company has used.',
    ...
    uNone: 'Nobody found.', uSearching: 'Searching…', uNoCompany: 'Not linked to any company.',
    uVerifyEmail: 'Verify email', uSendAccess: 'Resend access', uAccessSent: 'We emailed the person a code to reset their password.', uSendVerification: 'Send verification', uVerificationSent: 'Verification code sent to the email.', uMarkVerified: 'Mark verified', uVerified: 'email verified', uResetPassword: 'Set password',
    ...
    uDeleteHard: 'This person has not signed anything, so the account and their memberships are permanently deleted. The only way back is to register everything again.',
    ...
    uDeletePending: (n: number) => n === 1 ? 'Note: 1 review is open with this person — it goes back to the role, without stalling the investigation.' : `Note: ${n} reviews are open with this person — they go back to the role, without stalling the investigations.`,
  ...
  es: {
    ...
    seats: 'Usuarios (asientos)', seatsUsePlan: 'Usar el valor del plan',
    ...
    dangerBody: 'Suspender deja a TODA la empresa fuera de línea de inmediato: nadie entra y los enlaces públicos dejan de funcionar. Reactivar lo devuelve todo.',
    ...
    uDeleteHard: 'Esta persona no firmó nada, así que la cuenta y sus vínculos se eliminan de forma definitiva. La única vuelta atrás es registrar todo de nuevo.',
